...
public class SeKillController implements InitializingBean {
    ...
    public void verifyCode(TUser tUser, Long goodsId, HttpServletResponse response) {
        ...
        redisTemplate.opsForValue().set("captcha:" + tUser.getId() + ":" + goodsId, captcha.text(), 300,
                TimeUnit.SECONDS);
        try {
    ...
    public RespBean doSecKill(@PathVariable String path, TUser user, Long goodsId) {
        ...
        // 内存标记，减少Redis的访问,判断是否空库存
        if (EmptyStockMap.get(goodsId)) {
            return RespBean.error(RespBeanEnum.EMPTY_STOCK);
        }
        // 预减库存方法1，
        //Long stock = valueOperations.decrement("seckillGoods:" + goodsId);
        //预剑库存2，但是同一用户同时秒杀，数据库会进行回滚，而redis不会进行回滚，从而出现多减1的情况
    ...
    public String doSecKill2(Model model, TUser user, Long goodsId) {
        ...
        // 判断是否重复抢购
        TSeckillOrder seckillOrder = itSeckillOrderService
                .getOne(new QueryWrapper<TSeckillOrder>().eq("user_id", user.getId()).eq("goods_id", goodsId));
    ...
    public RespBean doSeckill(TUser user, Long goodsId) {
        ...
        // 判断是否重复抢购
        TSeckillOrder seckillOrder = itSeckillOrderService.getOne(new QueryWrapper<TSeckillOrder>().eq("user_id",
                user.getId()).eq(
                        "goods_id",
                        goodsId));
        if (seckillOrder != null) {
